...
import pde_adjoint_solver_neumann as model
...
#u = np.zeros((n-1)**2)
#def force(x, y):
#    return 6*(2*x-1)*(3*y**2-2*y**3)+6*(2*y-1)*(3*x**2-2*x**3)
#def true_solu(x, y):
#    return (3*x**2-2*x**3)*(3*y**2-2*y**3)-1/4

#u = np.zeros((n-1,n-1))
#for i in range(n-1):
#    for j in range(n-1):
#        u[i,j] = np.log(1+(j+0.5)/(n-1))
#u = u.reshape((-1))
#def force(x,y):
#    return (1+4*x)*y*(1-y)+2*x*(1-x**2)
#def true_solu(x,y):
#    return x*(1-x)*y*(1-y)
