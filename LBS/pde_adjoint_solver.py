...
class pde_adj_solver():
    ...
    def __init__(self, n, Sscale, SNR, force):  
        self.lnprob_likelihood = False
        ...
        self.idx_in = np.where(self.I < self.n**2-1)[0]
        self.Ibdy = np.concatenate((self.I[self.idx_in], np.ones(self.nbdy)*(self.n**2-1)))
    ...
    def solve(self, u):
        ...
        for i in range(batchsize):
            V = (self.Vphi*kappa[i,:,np.newaxis]).flatten()
            V = np.concatenate((V[self.idx_in], np.ones(self.nbdy)))
...
#    @profile    
    def lnprob(self, u, observation):
        ...
        for i in range(batchsize):
            ...
            duG = coo_matrix((p[i, self.J]*Vfull, (np.arange(0, 16*nface, 1)//16, self.I)), shape = ((nface, nvert)))
            ...
            duF[i, :] = -((1-self.lnprob_likelihood)*inv_Gamma_x_u[:, i]+1/(self.sigma**2)*duG.dot(spsolve(A.transpose(), r)))
